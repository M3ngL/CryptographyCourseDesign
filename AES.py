...
class AES:
    ...
    def round_key_generator(self, key):
        # 轮密钥产生
        ...
        for i in range(4, 44):
            ...
            w[i] = w[i-4] ^ temp
        return [long_to_bytes(sum([w[4 * i] << 96, w[4 * i + 1] << 64, w[4 * i + 2] << 32, w[4 * i + 3]])) for i in range(11)]

    def AddRoundKey(self, State, index):
        # 异或轮密钥
        # return self._16bytes_xor(State, self.RoundKeys[index])
        return xor(State, self.RoundKeys[index])
    ...
    def encrypt(self, plaintext):
        ...
        for i in range(0,len(State),16):
            ...
            for Round in range(1, 10):
                ...
                temp = self.MixColumns(temp)
                temp = self.AddRoundKey(temp, Round)
            temp = self.SubBytes(temp)
            temp = self.ShiftRows(temp)
            temp = self.AddRoundKey(temp, 10)
            res.append(b"".join(list(map(long_to_bytes,temp))))
        return b"".join(res)
    ...
    def decrypt(self, ciphertext):
        ...
        for i in range(0,len(State),16):
            temp = State[i:i + 16]
            for Round in range(1, 10):
                temp = self.ShiftRows_Inv(temp)
                temp = self.SubBytes_Inv(temp)
                temp = self.AddRoundKey(temp, 10 - Round)
                temp = self.MixColumns_Inv(temp)
            temp = self.ShiftRows_Inv(temp)
            temp = self.SubBytes_Inv(temp)
            temp = self.AddRoundKey(temp, 0)
            res.append(b"".join(list(map(long_to_bytes,temp))))
        return b"".join(res)
    
    def __init__(self,key):
        self.RoundKeys = self.round_key_generator(key)
...
if __name__ == "__main__":
    ...
    aes = AES(key)
    # 加密
    plaintext = b'12345600000000000000000000000000'
